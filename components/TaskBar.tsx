    ...
    <header className="flex h-12 items-center justify-between border-b border-zinc-800 bg-zinc-950 px-4">
        ...
        <nav className="flex items-center gap-1">
          {TABS.map((tab) => (
            <button
              ...
              className={`px-3 py-1 rounded text-sm font-medium transition-colors ${activeTab === tab.id
      ...
      <div className="relative flex items-center" ref={menuRef}>
        <button
          ...
          title="Settings"
          className={`flex h-8 w-8 items-center justify-center rounded transition-colors ${menuOpen ? "bg-zinc-700 text-zinc-100" : "text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100"
            }`}
        >
